...
public class FractalExplorer {
    ...
    public void createAndShowGUI() {
        ...
        Resetter resetHandler = new Resetter();                             //функционирование
        ...
        for (int x = 0; x < displaySize; x++) {                                     //цикл прохода по всем пикселям изображения
            for (int y = 0; y < displaySize; y++) {
                ...
                double yCoord = FractalGenerator.getCoord(range.y,
                        range.y + range.height, displaySize, y);            //генерация фрактала по пикселям и его вывод
    ...
    private class Chooser implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            ...
            if (source instanceof JComboBox) {
                ...
                assert fractal != null;                                                     //проверка что он не пустой
    ...
    private class Saver implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            if (e.getActionCommand().equals("Save")) {                      //Save - команда действия эту команду мы считываем с помощью getActionCommand
                ...
                FileFilter extensionFilter = new FileNameExtensionFilter(   //отображение файлов только с указанными типами
                        "PNG",
                        "png"
                );
                ...
                if (userSelection == JFileChooser.APPROVE_OPTION) {                  //если пользователь выбрал файл записываем его
                    java.io.File file = fileChooser.getSelectedFile();
                    String filePath = file.getPath();

                    if (!filePath.contains(".png")) file = new File(filePath + ".png"); //добавляет расширение в случае его отсутствия
